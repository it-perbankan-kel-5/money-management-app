                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table">
                                ...
                                <tbody>
                                    @foreach ($data as $budgeting)
                                        <tr>
                                            <td>{{ $budgeting['name'] }}</td>
                                            <td>{{ $budgeting['description'] }}</td>
                                            <td>{{ $budgeting['current_amount'] }}</td>
                                            <td>{{ $budgeting['limit_target'] }}</td>
                                            <td>{{ $budgeting['type'] }}</td>
                                            <td>

                                                <a href="{{ url('/budgeting/edit/' . $budgeting['id']) }}"
                                                    class="btn btn-warning btn-sm">
                                                    <i class="text-light far fa-edit"></i>
                                                </a>

                                                <button class="btn btn-danger btn-sm delete-button"
                                                    data-id="{{ $budgeting['id'] }}">
                                                    <i class="text-light far fa-trash-alt"></i>
                                                </button>
                                                <form action="{{ url('/budgeting/delete/{id}' . $budgeting['id']) }}"
                                                    method="POST" class="d-none" id="delete-form-{{ $budgeting['id'] }}">
                                                    @csrf
                                                    @method('DELETE')
                                                </form>
                                            </td>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            deleteButtons.forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    Swal.fire({
                        title: 'Konfirmasi Hapus budgeting',
                        text: 'Apakah Anda yakin ingin menghapus budgeting ini?',
                        icon: 'warning',
